...
AGORIC_CHAIN_KEYS=./_agstate/keys
...
# Stop agd if it is already running 
...
#rm -rf $AGORIC_CHAIN_DIR &> /dev/null
...
#if ! mkdir -p $AGORIC_CHAIN_DIR 2>/dev/null; then
#    echo "Failed to create chain folder. Aborting..."
#    exit 1
#fi
...
#$BINARY init test --home $AGORIC_CHAIN_DIR --chain-id=$CHAINID_2
echo "Initializing $CHAINID_1..."
osmosisd init test --home $CHAIN_DIR/$CHAINID_1 --chain-id=$CHAINID_1
echo "Initializing $CHAINID_3..."
gaiad init test --home $CHAIN_DIR/$CHAINID_3 --chain-id=$CHAINID_3

echo "Adding genesis accounts..."
echo $VAL_MNEMONIC_1 | osmosisd keys add val1 --home $CHAIN_DIR/$CHAINID_1 --recover --keyring-backend=test
#echo $VAL_MNEMONIC_2 | $BINARY keys add val2 --home $AGORIC_CHAIN_DIR --recover --keyring-backend=test
echo $VAL_MNEMONIC_3 | gaiad keys add val3 --home $CHAIN_DIR/$CHAINID_3 --recover --keyring-backend=test
echo $DEMO_MNEMONIC_1 | osmosisd keys add demowallet1 --home $CHAIN_DIR/$CHAINID_1 --recover --keyring-backend=test
#echo $DEMO_MNEMONIC_2 | $BINARY keys add provision --home $AGORIC_CHAIN_DIR --recover --keyring-backend=test
echo $DEMO_MNEMONIC_3 | gaiad keys add demowallet3 --home $CHAIN_DIR/$CHAINID_3 --recover --keyring-backend=test
echo $RLY_MNEMONIC_1 | osmosisd keys add rly1 --home $CHAIN_DIR/$CHAINID_1 --recover --keyring-backend=test 
#echo $RLY_MNEMONIC_2 | $BINARY keys add rly2 --home $AGORIC_CHAIN_DIR --recover --keyring-backend=test 
echo $RLY_MNEMONIC_3 | gaiad keys add rly3 --home $CHAIN_DIR/$CHAINID_3 --recover --keyring-backend=test 

osmosisd add-genesis-account $(osmosisd --home $CHAIN_DIR/$CHAINID_1 keys show val1 --keyring-backend test -a) 100000000000stake  --home $CHAIN_DIR/$CHAINID_1
#$BINARY add-genesis-account $($BINARY --home $AGORIC_CHAIN_DIR keys show val2 --keyring-backend test -a) 100000000000stake  --home $AGORIC_CHAIN_DIR
...
osmosisd add-genesis-account $(osmosisd --home $CHAIN_DIR/$CHAINID_1 keys show demowallet1 --keyring-backend test -a) 100000000000stake  --home $CHAIN_DIR/$CHAINID_1
#$BINARY add-genesis-account $($BINARY --home $AGORIC_CHAIN_DIR keys show provision --keyring-backend test -a) 1000000000000000ubld,500000000000000urun,100provisionpass,100sendpacketpass  --home $AGORIC_CHAIN_DIR
...
osmosisd add-genesis-account $(osmosisd --home $CHAIN_DIR/$CHAINID_1 keys show rly1 --keyring-backend test -a) 100000000000stake  --home $CHAIN_DIR/$CHAINID_1
#$BINARY add-genesis-account $($BINARY --home $AGORIC_CHAIN_DIR keys show rly2 --keyring-backend test -a) 100000000000ubld  --home $AGORIC_CHAIN_DIR
...
osmosisd gentx val1 7000000000stake --home $CHAIN_DIR/$CHAINID_1 --chain-id $CHAINID_1 --keyring-backend test
#$BINARY gentx val2 7000000000stake --home $AGORIC_CHAIN_DIR --chain-id $CHAINID_2 --keyring-backend test
gaiad gentx val3 7000000000stake --home $CHAIN_DIR/$CHAINID_3 --chain-id $CHAINID_3 --keyring-backend test
osmosisd collect-gentxs --home $CHAIN_DIR/$CHAINID_1
#$BINARY collect-gentxs --home $AGORIC_CHAIN_DIR
...
sed -i -e 's#"tcp://0.0.0.0:26656"#"tcp://0.0.0.0:'"$P2PPORT_2"'"#g' $AGORIC_CHAIN_DIR/config/config.toml
sed -i -e 's#"tcp://127.0.0.1:26657"#"tcp://0.0.0.0:'"$RPCPORT_2"'"#g' $AGORIC_CHAIN_DIR/config/config.toml
sed -i -e 's/timeout_commit = "5s"/timeout_commit = "1s"/g' $AGORIC_CHAIN_DIR/config/config.toml
sed -i -e 's/timeout_propose = "3s"/timeout_propose = "1s"/g' $AGORIC_CHAIN_DIR/config/config.toml
sed -i -e 's/index_all_keys = false/index_all_keys = true/g' $AGORIC_CHAIN_DIR/config/config.toml
sed -i -e 's/enable = false/enable = true/g' $AGORIC_CHAIN_DIR/config/app.toml
sed -i -e 's/swagger = false/swagger = true/g' $AGORIC_CHAIN_DIR/config/app.toml
sed -i -e 's#"tcp://0.0.0.0:1317"#"tcp://0.0.0.0:'"$RESTPORT_2"'"#g' $AGORIC_CHAIN_DIR/config/app.toml
sed -i -e 's#":8080"#":'"$ROSETTA_2"'"#g' $AGORIC_CHAIN_DIR/config/app.toml
...
sed -i -e 's/\"allow_messages\":.*/\"allow_messages\": [\"\/cosmos.bank.v1beta1.MsgSend\", \"\/cosmos.staking.v1beta1.MsgDelegate\"]/g' $CHAIN_DIR/$CHAINID_1/config/genesis.json
